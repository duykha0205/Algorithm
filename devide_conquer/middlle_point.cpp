...
float findMedian(int x[], int y[], int xl, int xr, int yl, int yr){

    int n = (xr - xl + 1) + (yr - yl + 1);

    if (n==0)
        return -1;

    cout  << "n: " << n << endl;
    bool flag_even = n%2 == 0 ? 1 : 0;

    int num_loop = (int)(n/2);
...
int main(){
    ...
    // cout << "res: " << findMedian(a, b, 0, 4, 0, 4) << endl;
    // auto t_end = std::chrono::high_resolution_clock::now();
    // double elapsed_time_ms = std::chrono::duration<double, std::milli>(t_end-t_start).count();
    // cout << elapsed_time_ms << endl;

    auto tstart = std::chrono::high_resolution_clock::now();
	cout << "res: " << findMedian( a, 5, b, 5 ) << endl;
    auto tend = std::chrono::high_resolution_clock::now();
    double elapsed_timems = std::chrono::duration<double, std::milli>(tend-tstart).count();
    cout << elapsed_timems << endl;
